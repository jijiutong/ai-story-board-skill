---
name: ai-story-board
description: Use when generating storyboards, visual development boards, character sheets, scene cards, posters, manga pages, mood boards, or any cinematic visual content from stories, novel excerpts, script fragments, or manga concepts. Triggers on requests like "storyboard this", "create a character sheet", "make a visual board", "漫剧", "分镜", "故事板", "视觉开发板", "转视频", "Seedance", "合并帧", "生成视频分镜图", "出视频 prompt". Full production pipeline: 50+ styles, 30+ relationships, 60+ color schemes, 12 narrative models, 12 emotion curves, 43 micro-expressions, 24 mood dimensions, 35+ composition rules, 80+ camera techniques, 40+ lighting schemes, 36 creatures, 36 environments, 46 props, 26 weather effects, 46 body language poses, 36 material textures, dialogue rhythm, 20-field character DNA, cultural accuracy across dynasties, 100+ AI error prevention, multi-platform (GPT Image 2-3/MJ v6-v7/SDXL-SD3/DALL-E 3/Luma/Pika), style fusion, video prompts, single-shot editing, audio reference, industry export, thumbnail boards, director notes, prompt scoring, continuity checking, multi-aspect, version control, style migration, prompt compression, batch chapter processing, AI video workflow (Seedance/Runway/可灵): storyboard is MASTER reference for timing/camera/transitions, character card prevents character drift, consolidated frames (2-3 per image) prevent visual drift, one integrated video prompt outputs complete video, video character reference cards with multi-angle faces/expression range/hand close-ups/DO NOT CHANGE markers, storyboard-to-video step-by-step guidance with post-production workflow (editing/voiceover/subtitles/sound/color grading).
---
...
▸ 辅助锚点 1：角色设定卡 — 防止角色跑偏
  → 回复"生成角色卡"或"格式3"
  → 作用：确保视频中角色面部/服装/武器不变形

▸ 辅助锚点 2：视频分镜图（3-4 张）— 防止画面跑偏
  → 回复"生成视频分镜图"或"合并帧"
...
【Seedance 视频生成流程】
1. 准备素材：故事板（主参考）+ 角色卡（角色锚点）+ 视频分镜图 3-4 张（画面锚点）
2. 生成视频 prompt（回复"出视频 prompt"）— AI 自动整合故事板全序列 + 锚点约束，生成一条完整视频 prompt
3. 将 prompt + 3 张参考图（角色卡作为第 1 参考，故事板作为主参考）输入 Seedance
...
- **出视频 prompt** → 整合故事板全序列+角色卡约束+分镜图锚点，生成一条完整视频 prompt，直接喂 Seedance 出完整视频
