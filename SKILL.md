...
【长视频跨段衔接 · 尾帧→首帧锁定】（多段视频拼接时使用，防止段落之间场景/角色瞬跳）
  ...
  尾帧截取策略（AI 根据故事板自动判断，不拘泥于固定帧数）：

  AI 读取故事板分段点所在的叙事位置，按叙事节奏决定尾帧截取：

  · 动作高潮断点 — 在挥砍/爆炸/撞击的峰值帧截取 1 帧，下一段从冲击瞬间开始
  · 情绪余韵 — 在角色表情变化顶峰后持留 2-3 帧，让情绪"沉下去"再切，下一段从余韵中渐入
  · 场景转场 — 在上一场景的最后一个完整构图截 1 帧，下一段以新场景开场，场景参考图随之切换
  · 匹配剪辑 — 截取上段最后一帧中与下段第一帧有视觉呼应关系的元素（形状/颜色/运动方向匹配）
  · 视角切换 — 上段 POV 角色视线方向的最后一帧作尾帧，下段从被注视对象/新 POV 开始
  · 时间跳跃 — 截取时间跳变前最后一帧，下段 prompt 声明「[X时间后]，光线/色调已变化」，但场景参考图不变
  · 并行叙事 — 两段同一时刻不同空间，截取上段最后一帧，下段用另一场景参考图，prompt 声明「同时间，另一地点」
  · 悬念切断 — 在疑问/威胁即将揭示的前一帧截断，下一段从揭示开始，给观众 1-2 秒悬念间隙
  · 声音先行 — 下段声音提前 0.5-1s 进入，画面随后跟上，截取上段尾帧时提示「下段声音已开始，画面延迟 0.5s 入」
  · 蒙太奇桥接 — 截取上段最后 3-5 帧作快速叠化，下段开头 3-5 帧同手法叠入，中间形成自然过渡带

  通用规则：
  → 尾帧永远是上段的最后一帧（或多帧），作为下段的首帧锚定参考图输入
  → 场景参考图/角色卡跨段复用，仅首帧锚定图更换为尾帧
  → Prompt 不必拘泥固定句式，AI 根据故事板分段点的叙事上下文，自动生成衔接描述
...
▸ 辅助锚点 1：场景参考图 — 防止场景跑偏（环境/光线/天空/氛围跳变）
  → 回复"生成场景参考图"或"场景锚点"，可选以下 7 种方法（可叠加）：
    ...
    · 720° 全景图 — 360°×180° 全景环境，覆盖所有拍摄方向
      → 回复"生成全景图"或"720全景"
      → 方式 A：直接生成全景图 | 方式 B：6 方向立方体贴图拼接 | 方式 C：实景拍摄
    · 环绕视频截图法 — 先生成环绕场景视频，再截取 8 个角度的关键帧
      → 回复"环绕截图"或"视频截图法"
      → 步骤：Seedance 生成 5s 环绕视频 → 截取 0°/45°/90°/.../315° 8 帧 → 拼为参考图
