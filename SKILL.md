...
▸ 辅助锚点 1：场景参考图 — 防止场景跑偏（环境/光线/天空/氛围跳变）
  ...
  → 可选格式：
    ...
    · 默认 — 单张全景环境图
  → 作用：锁死背景/天空/光线/阴影/环境细节跨帧一致不变

▸ 辅助锚点 2：首尾帧 — 防止人物动作/装扮/面容在长镜头中漂移
  → 回复"生成首尾帧"或"首尾帧"
...
【Seedance 视频生成流程】
1. 准备素材：故事板（主参考）+ 场景参考图（场景锚点，俯拍/4宫格/9宫格可选）+ 首尾帧（动作锚点，保持人物连贯）+ 角色卡（角色锚点）+ 视频分镜图 3-4 张（画面锚点）
2. 生成视频 prompt（回复"出视频 prompt"）— AI 自动整合故事板全序列 + 所有锚点约束，生成一条完整视频 prompt
3. 将 prompt + 参考图（场景参考图+首尾帧+角色卡+故事板+分镜图）输入 Seedance
...
- 场景一致性：引用场景参考图（背景/天空/光线/阴影/环境细节跨帧不变）
...
- **生成场景参考图 / 场景锚点** → 输出环境锚点图，可选俯拍图/4宫格/9宫格/单张全景，锁定场景地点+天气+光线+时间+环境关键元素+空间布局
- **生成首尾帧 / 首尾帧** → 输出帧1起始状态+帧N结束状态两张对比图，锁定动作起点终点防止人物漂移，适合长镜头/变装/大幅度动作
