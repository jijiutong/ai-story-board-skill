...
| 镜头数 | 短篇 7 镜，史诗 13 镜 |
| 主色调 | 2-4 个颜色 |
| **角色关系** | 见 `references/relationships.md`，提取关系类型 |
...
**角色关系加成匹配**（见 `references/relationships.md`）：
- 宿敌变恋人 → 强化对峙构图、冷暖分割光
- 双向暗恋 → 强化眼神交流、暧昧距离
- 师徒 → 强化仰视/俯视角度对比
- 年上/年下 → 强化体型差、身高差构图
- 先婚后爱 → 强化空间距离→逐渐靠近的镜头推进
...
**关系加成**：根据 `references/relationships.md` 中对应的关系类型，在 prompt 中追加构图建议和互动张力描述。
...
| `references/relationships.md` | 角色关系类型 + 构图建议 + 互动张力描述 |
