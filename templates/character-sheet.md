...
```text
电影级角色设定卡，主题《[片名]》，[主角/反派]：[角色姓名]。[画幅比例，默认 DEFAULT_ASPECT_RATIO（api-config.template.env）] 横版，专业角色设计参考板，用于 AI 视频角色一致性。版式样式：[LS编号. 版式名称]（见 engines/layout-styles.md）；默认 LS11 干净角色一致性卡，武侠/玄幻/黑金动作导演展示优先 LS12，科幻/机甲/神使优先 LS13。画面分为 6 个模块：
