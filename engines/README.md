...
→ asset-plan → consistency-trigger → reference-anchor → motion-physics → project-graph
→ video-prompt-assembly → consistency-engine → prompt-scorer
→ auto-repair → rules/final-video-qc → render-package
       ↑ 项目初始化/加载               ↑ 写入 state/ + 构建依赖图 + 一致性触达    ↑ 读取 state/ + 限定评估范围
```
...
| `consistency-trigger.md` | 一致性触达：分镜密度+场景方法+角色方法决策 → 输出保障建议（不阻塞主链） | asset-plan + state/ | reference-anchor |
| `reference-anchor.md` | 平台锚点：参考图策略+平台限制校验 → 写入 state/asset-map | consistency-trigger | motion-physics + state/asset-map |
...
| `final-video-qc.md` | 9项最终质检+致命项判定（含引用一致性+模板合规） |
