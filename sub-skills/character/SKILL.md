...
```
[角色名]服装武器细节参考卡。
左半：服装正面全身+背面，红色标注不可变细节（面料材质/颜色/纹样/配饰位置/腰带扣形制）。
右半：武器正面+侧面+细节特写，红色标注不可变细节（纹路/比例/材质/颜色/握柄形制）。
