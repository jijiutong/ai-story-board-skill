...
导演读取故事后，自动判断最佳时长。用户未指定时以此为默认：

| 时长 | 镜数 | 每镜 | 全案板图格数 | 叙事承载 | 适用场景 |
|------|------|------|-------------|----------|----------|
| **5-10s** | 2-3镜 | ~3s | 5格 | 单情绪节拍 / 瞬间揭示 | 一句话故事、氛围镜头、角色登场 |
| **15s** | 4镜 | ~3.75s | 7格 | 完整情绪弧线：起→承→转→合 | 标准场景、单次冲突、关系瞬间 |
| **30s** | 7-8镜 | ~4s | 9格 | 完整叙事 + 子节拍 + 可含1次闪回 | 故事短片、双人关系弧、悬疑揭示 |
| **60s+** | 12-15镜 | ~4s | 7格（选关键帧） | 多场景叙事、完整三幕 | 完整段落、多地点切换 |

**全案板图格数规则**：5-10s→5格 | 15s→7格 | 30s→9格 | 60s+→选7个关键帧。用户指定格数时以用户为准。

### 文档读取 → 时长切分

导演读取文字输入后，自动按字数判断时长。长文不硬塞，智能切分：

| 输入字数 | 建议时长 | 切分策略 |
|---------|---------|---------|
| **<100字** | 5-10s | 只有一个节拍，2-3镜够用 |
| **100-400字** | 15s | 标准场景，4镜弧线 |
| **400-1500字** | 30s | 完整故事，7-8镜展开 |
| **1500-5000字** | 选关键30s场景 | 多场景，先做一场最核心的。结尾提示"可继续下一场" |
| **5000+字** | 选一场15-30s | 先定位场景，输出后给全片分段建议 |
...
一键生成
  ...
  │
  ├── <100字 ──────────→ 变体 A（5-10s 快速版）
  ├── 100-400字 ───────→ 变体 B（15s 标准版）
  ├── 400-1500字 ──────→ 变体 C（30s 完整叙事版）
  ├── >1500字(单场) ──→ 变体 D（长文档多场景版）
  └── 用户指定 60s+ ──→ 变体 E（60s+ 多场景串联版）
```

用户说 `简洁` → 仍走对应变体，但输出只含模块 4+6+8（角色DNA + 分镜表 + 图像Prompt）。
用户说 `出视频 prompt` → Patch Pipeline：只要视频分支。
用户说 `出图` → Patch Pipeline：只要图像分支。
用户说 `一键全平台` → 变体 B/C 的 Prompt 输出三平台版本（GPT Image / MJ / SD）。
...
**适用**：100-400 字输入 · 15s/4镜 · 全案板图 7 格 · 角色/场景独立生成
...
**触发条件**：<100 字输入，或用户指定 5-10s

```
01 全案版生成 ──→ 02 全案版检测
                      │
      角色DNA + 场景DNA 内联嵌入（不独立生成角色卡/场景卡）
                      │
07 分镜版生成(2-3镜) ──→ 08 分镜连续性检测
                      │
09 图像 Prompt → 10 图像检测 → 11 视频 Prompt → 12 视频检测 → 13 交付
```

**变化**：
- 跳过 03/04（角色独立生成）和 05/06（场景独立生成）
- 角色 DNA / 场景 DNA 在 02 检测后直接内联写入分镜 Prompt，不单独出图
- Gate 08 检查项减半（镜数少，跨镜漂移风险低）
- 全案板图：5 格
- 输出不含「视频必配参考图」块（无独立角色卡/场景图）

#### 变体 B：15s 标准版（4镜）

**触发条件**：100-400 字输入，或用户未指定时长

→ 完整 13 步流水线（上方标准版）。全案板图 7 格。输出含参考图 @标注。

#### 变体 C：30s 完整叙事版（7-8镜）

**触发条件**：400-1500 字输入，或用户指定 30s

→ 完整 13 步。与标准版差异：

| 步骤 | 加强内容 |
|------|---------|
| Gate 02 | 增加叙事弧完整性检查（起承转合是否覆盖 7-8 镜） |
| Gate 08 | **闪回连续性专项**：如有闪回镜，检查闪回中人物/道具/光线是否与主时间线一致（参考闪回规则） |
| Gate 12 | 字数限制放宽至 ≤2000（镜数多）；如 ≥6镜，追加稳定性提示"可压缩为 5 镜" |
| 全案板图 | 9 格 |
...
```
Step 0: 场景切分（按地点/时间/角色进出识别边界）
         │
         ├── 场景 1（核心场景 · 30s）→ 变体 C Pipeline
         ├── 场景 2 → 用户确认后执行
         └── ...
         │
Step 跨场景衔接检测（所有场景完成后）:
  □ 角色跨场景外观一致
  □ 色彩系统跨场景统一
  □ 时间线无断裂
  □ 道具跨场景追踪（出场→使用→消失有交代）
...
```
检测到 [N] 个场景：
1. [场景名] — [地点] · [字数] → 建议 15s/4镜 或 30s/7镜
2. [场景名] — [地点] · [字数] → 建议 ...
...
优先制作场景 1。完成后回复"下一场"继续。
```

#### 变体 E：60s+ 多场景串联版（12-15镜）

**触发条件**：用户明确要求 60s+，或故事 >1500 字且要求完整视频

```
Step 0: 全片总案
  角色DNA（全局唯一） + 色彩系统（全局统一）
         │
Step 1: 场景拆分 → 每场子 Pipeline（走变体 C · 30s 或变体 B · 15s）
         │
         ├── 场景 1 → 子 Pipeline → 输出
         ├── 场景 2 → 子 Pipeline → 输出
         └── ...
         │
Step 跨场一致性检测（所有场景完成后）:
  □ 角色跨场外观连续（含换装标记）
  □ 道具跨场追踪（出场→使用→消失有交代）
  □ 时间线无断裂
  □ 情绪曲线跨场不跳变
...
**关键规则**：
- **不一次性生成全部 Prompt**。先输出「全片场景拆分表 + 每场核心任务」，建议先生成第 1 场
- 角色 DNA 全片只生成一次，跨场共享。每场只新增或更新场景 DNA
- 每场完成后执行子 Gate 检测；全部完成后执行跨场检测
- 全案板图：选 7 个全片关键帧

**拆分表输出格式**：
```
全片拆分 · 共 [N] 场
1. [场景名] — [地点] · [情绪] → 建议 30s/7镜
2. [场景名] — [地点] · [情绪] → 建议 15s/4镜
...
全局角色：[角色名]（跨场不变）
全局色彩：[主色调]（跨场统一）
...
完整 53 种风格 → 说 "看全部" 展示（完整列表见 ai-visual-director 主 Skill 的 `references/styles.md`）。
...
用户回复 1/2/3 确认风格 → 进入对应变体 Pipeline（**正常模式：每 Gate 暂停等待确认**），按字数自动路由到变体 A/B/C/D，每个 Gate 通过后输出当前结果 + 下一步选项。详见 [Pipeline 交互节点](#pipeline-交互节点)。
用户说 `一键生成` → 跳过 Step 2 推荐，自动选主风格 + 自动判断时长路由，全流程不暂停，直接出完整 11 模块全案板。
用户说 `一步一步来` → 在任意模式中强制切回每 Gate 暂停确认模式。
用户说 `简洁` → 进入对应变体但输出精简（仅模块 4+6+8）。
...
**分镜数量规则**：
- 默认 4 镜，时长默认 15s（用户未指定时）
- 用户指定镜数/时长时以用户为准

> **默认镜数**：15s/4镜（每镜约 3.75s，AI 视频稳定且叙事节奏舒适）。全案板图格数按时长自动判断（15s 默认 7 格），详见 Pipeline 时长表。

**画幅规则**：
- 图像版（GPT Image / MJ / SD）→ 默认 16:9
- 电影版（Seedance / Runway / Kling）→ 默认 2.39:1
...
【7. 视频完整 Prompt】（从分镜表输出 · 默认4镜/15s · 画幅 2.39:1 · Seedance / Runway / 可灵）
...
中文 Prompt：...
```
5. 视频完整可复制连续版（≤1500字）和平台压缩版（≤1000字）为衍生版本，默认不输出。用户可通过「输出连续版」/「输出平台压缩版」指令单独获取。
...
- 平台压缩版（按需输出）≤1000 字，去掉场景重复描述，仅保留角色DNA+画面+运镜+转场
...
- **视频完整可复制连续版**（≤1500字，中文）：用户输入 `输出连续版` 时触发。所有镜头合并为一条连续中文 prompt。
- **视频完整可复制英文版**（≤1500字）：用户输入 `输出英文版` 时触发。所有镜头合并为一条连续英文 prompt，含角色DNA+场景DNA+运镜+转场+色彩。
- **平台压缩版**（≤1000字）：用户输入 `输出平台压缩版` 时触发。去掉场景重复描述、焦段信息、色彩渐变细节，只保留角色DNA片段 + 每镜关键画面描述 + 运镜 + 转场 + 核心负面词。标注 `【平台压缩版 · 直接复制到 Seedance / Runway / 可灵】`。如15s视频镜数≥6，建议标注"稳定性提示：可压缩为5镜"。
